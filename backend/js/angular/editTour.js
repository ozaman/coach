...
app.controller("pro",  function($scope,$location,$http,$window,$cookies,Upload) {
        ...
        $http({
                ...
                headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                 ...
                 console.log(res)
                 $scope.tours = res[0];
                 console.log(res[0].show_recommend)
                 console.log(res[0].show_popular)
                   $scope.echekpopulars = res[0].show_popular;
                    $scope.echekrecommends = res[0].show_recommend;
                 if (res[0].show_recommend == 1) {
                        $scope.eechekrecommend = true;
                 }
                 else{
                        $scope.eechekrecommend = false;

                 }
                  if (res[0].show_popular == 1) {
                        $scope.eechekpopular = true;
                 }
                 else{
                        $scope.eechekpopular = false;
  ...
  $scope.deletePackage = function(data){
    ...
    $scope.eprogramchange =function(x){
        //alert('aaaa')
        $scope.pro = x;
    ...
    $scope.saveEditpackage =function(){
       ...
       // console.log(chekpopular+'\n'+chekrecommend)
        ...
        console.log($scope.echekpopulars)
        console.log($scope.echekrecommends)
         if($scope.pro == ''|| $scope.pro == undefined){
            $scope.pro = $scope.eprogram;
         }
         $http({
                method : 'POST',
